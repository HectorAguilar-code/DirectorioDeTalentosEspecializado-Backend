  ...
  try {
    ...
    if (!idUsuario || idUsuario.length != 24) {
      return res.status(400).json({
        ...
        msg: `${
          idUsuario
            ? "El _idUsuario no es valido"
            : "No se recibio un identificador de usuario"
        }`,
        cont: {
          idUsuario,
        },
    ...
    const obtenerUsuarios = await UsuarioModel.findById(idUsuario);
    if (obtenerUsuarios.length < 1) {
      return res.status(400).json({
        ok: false,
        msg: "No se encontrarón productos en la base de datos",
    ...
    return res.status(200).json({
      ok: true,
      msg: "Se obtuvo el usuario de manera correcta",
      cont: {
        obtenerUsuarios,
      },
    });
...
app.post("/", async (req, res) => {
  ...
  try {
    const body = {
      ...req.body,
      strContrasena: req.body.strContrasena
        ? bcrypt.hashSync(req.body.strContrasena, 10)
        : undefined,
    };
    const bodyUsuario = new UsuarioModel(body);
    const encontrarEmailUsuario = await UsuarioModel.findOne({
    ...
    if (req.files) {
      ...
      bodyUsuario.strImagen = await subirArchivo(
        req.files.strImagen,
        "usuario",
    ...
    if (!req.body._idObjRol) {
      const encontroRolDefault = await RolModel.findOne({
        blnRolDefault: true,
      });
  ...
  try {
    ...
    if (!_idUsuario || _idUsuario.length != 24) {
      return res.status(400).json({
        ok: false,
        msg: _idUsuario
          ? "El id no es valido, se requiere un id de almenos 24 caracteres"
    ...
    const encontroUsuario = await UsuarioModel.findOne({
      _id: _idUsuario,
      blnEstado: true,
    });
    ...
    const encontroCorreo = await UsuarioModel.findOne(
      { strCorreo: req.body.strCorreo, _id: { $ne: _idUsuario } },
      { strNombre: 1, strCorreo: 1 }
    );
    ...
    const actualizarUsuario = await UsuarioModel.findOneAndUpdate(
      { _id: _idUsuario },
      {
        $set: {
          ...
          nmbTelefono: req.body.nmbTelefono,
...
app.delete("/", async (req, res) => {
  try {
    ...
    const modificarEstadoUsuario = await UsuarioModel.findOneAndUpdate(
      { _id: _idUsuario },
      { $set: { blnEstado: blnEstado } },
      { new: true }
    );
    ...
    return res.status(200).json({
      ...
      msg:
        blnEstado == true
          ? "Se activo el usuario de manera exitosa"
          : "Se desactivo el usuario de manera exitosa",
      cont: {
